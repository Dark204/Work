function [test_gmean,test_AUC,test_balance] = ELM_regularized_LXL2(TrainingData_File, TestingData_File, Elm_Type, NumberofHiddenNeurons, ActivationFunction,C)
...
for i = 1:length(T)
    if T(i) == 1
        W(1,i)= 0.618*(length(T)-length(posx))/length(posx);
    else
        W(1,i)= 1;
    end 
end
